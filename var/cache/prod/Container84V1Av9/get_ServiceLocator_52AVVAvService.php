<?php

use Symfony\Component\DependencyInjection\Argument\RewindableGenerator;
use Symfony\Component\DependencyInjection\Exception\RuntimeException;

// This file has been auto-generated by the Symfony Dependency Injection Component for internal use.
// Returns the private '.service_locator.52AVVAv' shared service.

return $this->privates['.service_locator.52AVVAv'] = new \Symfony\Component\DependencyInjection\ServiceLocator(['bus' => function () {
    return ($this->services['message_bus'] ?? $this->load('getMessageBusService.php'));
}, 'flashBag' => function () {
    return ($this->privates['session.flash_bag'] ?? ($this->privates['session.flash_bag'] = new \Symfony\Component\HttpFoundation\Session\Flash\FlashBag()));
}, 'formFactory' => function () {
    return ($this->services['form.factory'] ?? $this->load('getForm_FactoryService.php'));
}, 'repository' => function () {
    return ($this->privates['MsgPhp\\User\\Infra\\Doctrine\\Repository\\UserRepository'] ?? $this->getUserRepositoryService());
}, 'twig' => function () {
    return ($this->services['twig'] ?? $this->getTwigService());
}]);
