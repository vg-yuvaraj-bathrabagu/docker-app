...
INSERT INTO mpp_status (\`nodes\`, \`category\`, \`status\`, \`module\`, \`comments\`, \`ts_begin\`, \`description\`, \`uuid\`, \`accountid\`) VALUES ("$6", "$5", "Created", "test_mod", "test_comments", now(), "test_desc", "$7", "$8");
EOF
...
mysql -h $1 --user=$2 --password=$3 $4 <<EOF
UPDATE mpp_status set \`statusresult\` = '$https_url' where \`uuid\` = "$7";
EOF
