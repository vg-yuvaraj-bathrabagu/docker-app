...
UPDATE hadoop_status set \`status\` = 'Starting', \`ts_begin\` = NOW(), \`color\` = 'orange',\`comments\` = CONCAT('start kicked off at ', ' ', NOW()), \`statusresult\` = ''  where id = $5;
EOF
echo "uuid" $6
...
mysql -h $1 --user=$2 --password=$3 $4 <<EOF
UPDATE hadoop_status set status = 'Started', \`ts_begin\` = NOW(), \`color\` = 'green', \`comments\` = CONCAT('completed at ', ' ', NOW()), \`statusresult\` = '$https_url' where id = $5;
EOF
echo 0;