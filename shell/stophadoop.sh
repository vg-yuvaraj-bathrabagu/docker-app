...
UPDATE hadoop_status set \`status\` = 'Stopping', \`ts_begin\` = NOW(), \`color\` = 'orange', \`comments\` = CONCAT('completed at ', ' ', NOW()), \`statusresult\` = '' where id = $5;
EOF
echo "uuid" $6
...
mysql -h $1 --user=$2 --password=$3 $4 <<EOF
UPDATE hadoop_status set \`status\` = 'Stopped', \`ts_begin\` = NOW(), \`color\` = 'red', \`comments\` = CONCAT('completed at ', ' ', NOW()), \`statusresult\` = '$https_url' where id = $5;
EOF
echo 0;
